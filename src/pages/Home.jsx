...
function Home() {
  ...
  useEffect(() => {
    try {
      ...
      axios.get(`${import.meta.env.VITE_BACKEND_URL}/dishes`).then((resp) => {
        setDishes(resp.data);
      });
    } catch (error) {
      navigate("/error");
  ...
  return (
    <div className="hero-section-container">
      <div
        className={
          isDarkTheme ? "hero-section darkTheme" : "hero-section lightTheme"
        }
        style={{ backgroundColor: isDarkTheme ? "#212529" : "#fff" }}
      >
        <h5 style={{ color: isDarkTheme ? "#fff" : "black" }}>
          Thanks to Crudo you can create or save your daily meal plans and
          individual dishes.
        </h5>
        <img src={hero} alt="hero section" width="300px" />
        <p style={{ color: isDarkTheme ? "#fff" : "black" }}>
          Also, you can see how prepare a dish
        </p>
        <Button as={Link} to="/preparation/NBTkyjJ" variant="primary">
          {" "}
          See a preparation demo
        </Button>
      </div>
      <div>
      ...
      <div>
        ...
        <div className="d-flex m-2 gap-2 justify-content-center align-items-center flex-wrap">
          {dishes.map((eachDish) => {
            return (
              eachDish.isFav && <DishCard key={eachDish.id} dish={eachDish} />
            );
          })}
        </div>
      </div>
